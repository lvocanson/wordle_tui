#!/usr/bin/env bash
# wtui-ship.sh — build the shipping binary: the smallest this project knows how to produce.
#
#   ./wtui-ship.sh                                    # build for the host
#   ./wtui-ship.sh run                                # build it and run the game
#   ./wtui-ship.sh --target x86_64-unknown-linux-musl
#   ./wtui-ship.sh -n                                 # print the cargo command, run nothing
#   ./wtui-ship.sh build -v --features …              # anything unrecognised goes to cargo
#
# There is no profile option, on purpose. The other way to build this project is a plain
# `cargo build --release` on whatever toolchain you already have — no nightly, no rust-src, no
# script. This is the opposite end, everything on, and it installs what that needs:
#
#   * platform — the target triple defaults to the host's (from `rustc -vV`), and the linker flags
#     follow from it: MSVC folds through /OPT:ICF (already in .cargo/config.toml) and emits /MAP,
#     ELF needs an explicit lld + --icf=all and emits -Wl,-Map.
#   * toolchain — installs the pinned nightly AND its rust-src component when missing.
#     `cargo +nightly-…` on its own does auto-install a toolchain, but with the default profile,
#     i.e. without rust-src, which is exactly what the build-std in .cargo/config.toml needs. Any
#     --target you ask for that is not installed is added too (that is the musl prerequisite in
#     BUILD.md).
#   * flags — passed as `--config target.<triple>.rustflags=[…]`, which MERGES with the [target]
#     blocks of .cargo/config.toml. An env RUSTFLAGS OVERRIDES them instead, which is why the
#     hand-typed BUILD.md lines have to repeat /OPT:ICF, /DEBUG:NONE and --build-id=none, and this
#     does not.
#
# Options: --target <triple>, -n/--dry-run, -h/--help. The first bare word is the cargo
# subcommand (default `build`); everything else is forwarded to cargo untouched.
#
# Run from anywhere (it cd's to the repo root). Use Git Bash on Windows, like tools/validate.sh:
#   bash wtui-ship.sh run

set -uo pipefail

REPO="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
cd "$REPO" || exit 1

# Pinned measurement toolchain: binary sizes are only comparable at equal rustc (same pin as
# tools/validate.sh, .github/workflows/ci.yml and BUILD.md). Bump it in all four places, then
# re-measure the reference totals.
NIGHTLY='nightly-2026-08-25'

say()  { printf '\033[1;36m%s\033[0m\n' "$*"; }
die()  { printf '%s\n' "$*" >&2; exit 2; }
have() { command -v "$1" >/dev/null 2>&1; }
usage(){ awk 'NR > 1 && !/^#/ { exit } NR > 1 { sub(/^# ?/, ""); print }' "$0"; }

# --- arguments ---------------------------------------------------------------------------------
CMD=''
TARGET=''
DRY=0
EXTRA=()

while [ $# -gt 0 ]; do
  case "$1" in
    --target)             [ $# -ge 2 ] || die "--target needs a triple"; TARGET="$2"; shift ;;
    --target=*)           TARGET="${1#*=}" ;;
    -n|--dry-run)         DRY=1 ;;
    -h|--help)            usage; exit 0 ;;
    --stable|--build-std|--ship|--immediate-abort)
      die "$1 is gone: this script only builds the shipping profile. For the no-prerequisites build, run \`cargo build --release\` (see BUILD.md)." ;;
    --)                   EXTRA+=("$@"); break ;;              # the rest belongs to the program
    -*)                   EXTRA+=("$1") ;;                     # unknown flag -> cargo's problem
    *)                    if [ -z "$CMD" ]; then CMD="$1"; else EXTRA+=("$1"); fi ;;
  esac
  shift
done
CMD="${CMD:-build}"

# --- platform ----------------------------------------------------------------------------------
have rustup || die "rustup not found — install it from https://rustup.rs"
HOST="$(rustc -vV | sed -n 's/^host: //p')"
[ -n "$HOST" ] || die "could not read the host triple from 'rustc -vV'"
TARGET="${TARGET:-$HOST}"

case "$TARGET" in
  *-windows-msvc) LINKER=msvc ;;
  *-linux-*)      LINKER=elf ;;
  *)              LINKER=other ;;   # everything else builds fine, just without the ICF extras
esac

# --- toolchain ---------------------------------------------------------------------------------
TC=(--toolchain "$NIGHTLY")

# A dry run stays side-effect free: with -n, nothing below installs anything.
if [ "$DRY" -eq 0 ]; then
  if ! rustup toolchain list | grep -q "^${NIGHTLY}"; then
    say "installing $NIGHTLY + rust-src (the build-std in .cargo/config.toml needs it)"
    rustup toolchain install "$NIGHTLY" --profile minimal --component rust-src \
      || die "could not install $NIGHTLY"
  elif ! rustup component list "${TC[@]}" --installed | grep -q '^rust-src'; then
    say "adding rust-src to $NIGHTLY (the build-std in .cargo/config.toml needs it)"
    rustup component add rust-src "${TC[@]}" || die "could not add rust-src to $NIGHTLY"
  fi

  if ! rustup target list "${TC[@]}" --installed | grep -qx "$TARGET"; then
    say "adding target $TARGET to $NIGHTLY"
    rustup target add "${TC[@]}" "$TARGET" || die "could not add target $TARGET"
  fi
fi

# --- flags -------------------------------------------------------------------------------------
# Only what .cargo/config.toml does not already carry: its [target] blocks stay in force because
# these go through --config (merge) and not RUSTFLAGS (override).
FLAGS=(-Cpanic=immediate-abort --cfg immediate_abort)

# The nightly toolchain ships its own lld (rust-lld), so ELF ICF needs no package here.
[ "$LINKER" = elf ] && FLAGS+=(-Clinker-features=+lld -Clink-arg=-Wl,--icf=all)

# Byte-neutral: makes the link also emit its symbol map for `cargo run --example bloat`
# (OPTIMIZATION.md "Symbol attribution"). Only for the two triples tools/validate.sh reads.
case "$TARGET" in
  *-windows-msvc)      FLAGS+=(-Clink-arg=/MAP:target/wordle_tui.map) ;;
  *-unknown-linux-gnu) FLAGS+=(-Clink-arg=-Wl,-Map=target/wordle_tui-linux.map) ;;
esac

# Every flag above is unstable; -Zunstable-options gates them.
FLAGS=(-Zunstable-options "${FLAGS[@]}")

# --- command -----------------------------------------------------------------------------------
toml_array(){ local out='[' sep=''; for f; do out="$out$sep\"$f\""; sep=', '; done; printf '%s]' "$out"; }

# The build-std in .cargo/config.toml REQUIRES an explicit --target, so one is always passed and
# the binary always lands under target/<triple>/release/.
ARGS=(cargo "+$NIGHTLY" "$CMD" --release --target "$TARGET"
      --config "target.$TARGET.rustflags=$(toml_array "${FLAGS[@]}")")
ARGS+=(${EXTRA[@]+"${EXTRA[@]}"})

EXT=''; case "$TARGET" in *windows*) EXT='.exe' ;; esac
DIR="${CARGO_TARGET_DIR:-target}"                     # tools/validate.sh redirects it under WSL
OUT="$DIR/$TARGET/release/wordle_tui$EXT"

show(){ local a out=''; for a; do case "$a" in *[\ \"\[]*) out="$out '$a'" ;; *) out="$out $a" ;; esac; done; printf '%s\n' "${out# }"; }
say "target: $TARGET | toolchain: $NIGHTLY"
show "${ARGS[@]}"
[ "$DRY" -eq 1 ] && exit 0

"${ARGS[@]}"; RC=$?
if [ "$RC" -eq 0 ] && [ "$CMD" = build ] && [ -f "$OUT" ]; then
  say "$OUT — $(wc -c < "$OUT" | tr -d ' ') bytes"
fi
exit "$RC"
