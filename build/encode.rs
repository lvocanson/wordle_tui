...
// Encode the whole corpus under one (order, inc) setting and return the resulting blob.
fn encode_corpus(corpus: &[TaggedWord], answers: u32, word_len: usize, order: usize, inc: u16) -> Vec<u8> {
    let mut enc = RangeEncoder::new();
    let mut model = Model::new(word_len, order, inc);
    let mut prev: Option<&[u8]> = None;
    let (mut remaining, mut remaining_a) = (corpus.len() as u32, answers);
...
// Search the whole model space and keep whatever compresses the corpus smallest. The winning
...
pub fn best_model(corpus: &[TaggedWord], answers: u32, word_len: usize) -> (usize, u16, Vec<u8>) {
    let mut best_order = 1;
    let mut best_inc = 1;
    let mut best_blob = encode_corpus(corpus, answers, word_len, best_order, best_inc);
    for order in 1..=MAX_ORDER {
        for inc in 1..=MAX_INC {
            let blob = encode_corpus(corpus, answers, word_len, order, inc);
